...
# This shader supplies only the native Metal pass's RD interface metadata.
# Validating its SPIR-V does not compile or validate the handwritten MSL kernel.
native_interface=(ROOT/'servers/rendering/renderer_rd/shaders/kiln_specular_native.glsl').read_text().replace('#[compute]','').replace('#VERSION_DEFINES','')
glsl=args.output/'kiln_specular_native_interface.comp';spv=glsl.with_suffix('.spv')

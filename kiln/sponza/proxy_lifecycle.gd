...
func _ready() -> void:
	...
	await settle()
	var initial := stats()
	assert(initial.proxy_triangles_unique < initial.source_triangles_unique)
	...
	mesh.radius = 0.7 # No explicit rebuild: the mesh's changed signal invalidates its proxy.
	await settle()
	var geometry := stats()
	assert(geometry.geometry_version > replaced.geometry_version and geometry.proxy_builds > replaced.proxy_builds)
	instance.position.x = 1.0 # Static transforms are detected too.
	await settle()
	var moved := stats()
	assert(moved.geometry_version > geometry.geometry_version)
	assert(moved.proxy_builds == geometry.proxy_builds)
	assert(gi.validate_bvh(128).passed)
	instance.visible = false
	await settle()
	var hidden := stats()
	assert(hidden.static_triangles == 0 and hidden.proxy_triangles_unique == 0)
	instance.visible = true
	await settle()
	assert(stats().static_triangles > 0)
	for divisor in [4, 2, 4]:
		gi.set_resolution_divisor(divisor)
		await settle()
		assert(stats().resolution_divisor == divisor)

...
func _ready() -> void:
	for arg in OS.get_cmdline_user_args():
		...
		if arg.begins_with("--gi-divisor="): gi_divisor = int(arg.get_slice("=", 1))
		if arg.begins_with("--proxy-ratio="): proxy_ratio = float(arg.get_slice("=", 1))
	...
	gi.set_resolution_divisor(gi_divisor)
	gi.set_proxy_quality(proxy_ratio, 0.001)
...
func _process(_delta: float) -> void:
	...
	if ticks % 30 == 0 and not tod_video:
		...
		hud.text = "SPONZA | %02d:%02d | %s | GI %s | %d FPS\nG GI   Space play/pause TOD   C camera   RMB + WASD/QE fly   F1 indirect\nProxy %d / %d triangles | imported surfaces %d | %s" % [int(hour), int(fmod(hour, 1.0) * 60), "SUN + SKY + LOCAL" if local_mode else "SUN + SKY ONLY", "ON" if gi.is_enabled() else "OFF", Engine.get_frames_per_second(), s.get("proxy_triangles_unique", 0), s.get("source_triangles_unique", 0), s.get("imported_proxy_surfaces", 0), s.get("backend", "")]
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		match event.keycode:
			...
			KEY_F1:
				debug_mode = 7 if debug_mode == 0 else 0
				ProjectSettings.set_setting("rendering/kiln/debug_view", debug_mode)
			KEY_ESCAPE:
				free_camera = false
...
func shot(label: String) -> void:
	...
	FileAccess.open(directory.path_join("lighting.json"), FileAccess.WRITE).store_string(JSON.stringify({"hour": fposmod(tod_phase * 24.0 + 6.0, 24.0), "sun_energy": sun.light_energy, "sky_energy": sky_energy_value, "local_mode": local_mode, "gi_enabled": gi.is_enabled(), "debug": debug_mode}))

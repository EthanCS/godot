param([switch]$ForwardPlus, [switch]$NoGI, [switch]$MultiLight, [switch]$TwoBounce, [switch]$DryFloor, [switch]$NoSpecular, [switch]$NRD, [switch]$NoNRD, [switch]$FullSpecularRate, [switch]$Still, [ValidateRange(0.02, 1.0)][float]$Roughness = 0.18, [ValidateRange(0, 28)][int]$DebugView = 0)
...
if ($NRD -and -not $NoNRD) { $demoArgs += '--nrd' }
